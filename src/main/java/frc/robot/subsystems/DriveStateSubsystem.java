...
public class DriveStateSubsystem extends SubsystemBase {
    ...
    public DriveStateSubsystem(CommandSwerveDrivetrain commandSwerveDrivetrain, XboxController xbox) {
        this.xbox = xbox;
        this.commandSwerveDrivetrain = commandSwerveDrivetrain;
        this.lastChassisSpeeds = commandSwerveDrivetrain.getChassisSpeeds();
    ...
    public void periodic() {
        ChassisSpeeds speed = commandSwerveDrivetrain.getChassisSpeeds();
        ChassisAccelerations accelerations = new ChassisAccelerations(lastChassisSpeeds, speed,
                System.currentTimeMillis() - lastSpeed);
        lastSpeed = System.currentTimeMillis();
        lastChassisSpeeds = speed;
        ...
        switch (commandSwerveDrivetrain.getRotationStyle()) {
            ...
            case Aimbot -> {
                Pose3d hubPose3d = FieldConstants.getHubPosition();
                Pose2d hubPose = hubPose3d.toPose2d();
                ...
                if (robotPoseOpt.isPresent()) {
                    ...
                    double targetOffset = hubPose.rotateAround(robotPose.getTranslation(), robotPose.getRotation())
                    ...
                    if (solution != null) {
                        offsetX = robotPose.getX() - solution.effectiveTargetPose().getX(); // Long Side
                        offsetY = robotPose.getY() - solution.effectiveTargetPose().getY(); // Short Side
                    } else {
                        offsetX = robotPose.getX() - hubPose.getX(); // Long Side
                        offsetY = robotPose.getY() - hubPose.getY(); //
                    }
                    goalDegrees = MathUtil.angleModulus(Math.atan2(offsetY, offsetX) + Math.PI);
                    double goalZSpeed = DriveConstants.AIMBOT_CONTROLLER
                            .calculate(robotPose.getRotation().getRadians(), goalDegrees);
                    ...
                    velocity[0] = xSpeed;
                    velocity[1] = ySpeed;
                    velocity[2] = goalZSpeed * DriveConstants.MAX_ROBOT_RAD_VELOCITY;
