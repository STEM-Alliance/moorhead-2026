...
public class BallSimulator {
    ...
    private final NetworkTable networkTable = NetworkTableInstance.getDefault().getTable("Ball Simulator");
    private final StructArrayPublisher<Pose3d> ballPosePublisher = networkTable.getStructArrayTopic("ball_poses", Pose3d.struct).publish();
    ...
    public void update() {
        ...
        if (balls.isEmpty()) {
            ballPosePublisher.set(new Pose3d[0]);
            return;
        }
        ...
        ballPosePublisher.set(getBallPoses());
    }
